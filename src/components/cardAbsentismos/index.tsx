  ...
  return (
    <Wrapper>
      <Container status={status}>
        ...
        <InfoText>Fecha de Inicio: {fechaInicio}</InfoText>
        <InfoText>Fecha de Vuelta: {fechaVuelta}</InfoText>
        <InfoText>Motivo: {motivo}</InfoText>
      </Container>
      <ButtonContainer>
        {status === 'Pendiente de Aprobación' ? (
          <>
            <Button color="blue" onClick={onEditar}>Editar Solicitud</Button>
            <Button color="red" onClick={onCancelar}>Cancelar Solicitud</Button> {/* Botón de cancelar */}
          </>
        ) : (
